...
namespace TestEnvironment.Docker.Containers.Mongo
{
    public class MongoSingleReplicaSetContainer : Container, IMongoContainer
    {
        ...
        protected override ContainerConfiguration GetContainerConfiguration(IDictionary<string, string> environmentVariables, IList<string> exposedPorts)
        {
            var configuration = base.GetContainerConfiguration(environmentVariables, exposedPorts);

            if (_port != null)
            {
                // custom mongo port isn't exposed by default from docker container
                configuration.ExposedPorts = new List<string> { $"{_port.Value}/tcp" };
            }
